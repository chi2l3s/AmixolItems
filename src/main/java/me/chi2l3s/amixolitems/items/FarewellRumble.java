...
public class FarewellRumble {

    private final AmixolItems plugin;

    public FarewellRumble(AmixolItems plugin) {
        this.plugin = plugin;
    }

    public ItemStack itemStack(){
        String materialString = plugin.getConfig().getString("farewellrumble.material");
        String name = plugin.getConfig().getString(ColorUtil.message(ChatColor.translateAlternateColorCodes('&',"farewellrumble.display-name")));
        List<String> lore = plugin.getConfig().getStringList("farewellrumble.lore");
