...
public class ExplosiveStick {

    private final AmixolItems plugin;

    public ExplosiveStick(AmixolItems plugin) {
        this.plugin = plugin;
    }

    public ItemStack itemStack(){
        String materialString = plugin.getConfig().getString("explosivestick.material");
        String name = plugin.getConfig().getString(ColorUtil.message(ChatColor.translateAlternateColorCodes('&',"explosivestick.display-name")));
        List<String> lore = plugin.getConfig().getStringList("explosivestick.lore");
